...
class Usuario(Base):
    ...
    preferencias = relationship(
        "PreferenciasUsuario", back_populates="usuario", uselist=False,
    )
    tokens_sesion = relationship("TokenSesion", back_populates="usuario")
    tokens_restablecimiento = relationship(
        "TokenRestablecimiento", back_populates="usuario",
    )

    def __repr__(self):
        return f"<Usuario(id={self.id}, nombre='{self.nombre_completo}', email='{self.email}')>"
